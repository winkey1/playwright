...
# --- KONFIGURASI SCRIPT ---
TARGET_URL = "https://browserleaks.com/webrtc"
...
def get_valid_devices(playwright_instance):
    ...
    return [name for name in USER_REQUESTED_DEVICES if name in supported_devices]
...
def run_instance(instance_id: int, proxy_str: str, device_name: str):
    ...
    try:
        with sync_playwright() as p:
            ...
            # STRUKTUR YANG BENAR UNTUK MEMANGGIL FUNGSI
            context = p.chromium.launch_persistent_context(
                # Argumen 1: Path ke folder profil (wajib)
                "user_data_dir_instance_" + str(instance_id),
                
                # Argumen selanjutnya adalah 'keyword arguments' yang valid
                headless=False,
                proxy={
                ...
                # 'Unpacking' device_config untuk memasukkan semua opsinya
                **device_config
            )
            
            page = context.new_page()
            page.wait_for_timeout(2000) # Beri waktu ekstensi untuk aktif
...
if __name__ == "__main__":
    # ... (Sisa kode ini tidak perlu diubah)
    if not PROXY_LIST or not PROXY_LIST[0]:
        ...
        if not VALID_DEVICES:
            ...
            for i in range(NUM_INSTANCES):
                ...
                thread.start()
                time.sleep(5) # Jeda sedikit lebih lama untuk peluncuran dengan ekstensi
